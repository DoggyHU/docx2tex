...
{
    Console.Error.WriteLine("Usage: Docx2Tex <input.docx>");
    return 1;
}

string fileName = Path.GetFileNameWithoutExtension(docxPath);
string safeName = Regex.Replace(fileName, @"[^\w\- ]", "_");
string outputDir = Path.Combine(Path.GetDirectoryName(docxPath)!, $"{safeName}-tex");
    ...
    else if (element is Table table)
    {
        ...
        string GetRunText(Run r, out double fontSizePt)
        {
            ...
            if (!string.IsNullOrEmpty(fs) && int.TryParse(fs, out int hv))
                fontSizePt = hv / 2.0;
            txt = EscapeLatex(txt);
            if (va == VerticalPositionValues.Superscript) txt = $"$^{{{txt}}}$";
            else if (va == VerticalPositionValues.Subscript) txt = $"$_{{{txt}}}$";
            if (bold && italic) txt = $"\\textbf{{\\textit{{{txt}}}}}";
            else if (bold) txt = $"\\textbf{{{txt}}}";
            else if (italic) txt = $"\\textit{{{txt}}}";
            return txt;
        }
        ...
        for (int c = 0; c < maxCol; c++) colAlign[c] = "l"; // 默认左对齐
        var alignCounts = new Dictionary<int, Dictionary<string, int>>();
        for (int ri = 0; ri < rawRows.Count; ri++)
        {
            int pos = 0;
            for (int ci = 0; ci < rawRows[ri].Count && pos < maxCol; ci++)
            {
                ...
                int colSpan = cr.C;
                string a = cr.A;
                if (string.IsNullOrEmpty(a)) a = "l";
                for (int j = 0; j < colSpan && pos + j < maxCol; j++)
                {
                    if (!alignCounts.ContainsKey(pos + j))
                        alignCounts[pos + j] = new Dictionary<string, int>();
                    var d = alignCounts[pos + j];
                    d[a] = d.TryGetValue(a, out int v) ? v + 1 : 1;
                }
                pos += colSpan;
            }
        }
        ...
        {
            if (alignCounts.TryGetValue(c, out var d) && d.Count > 0)
                colAlign[c] = d.OrderByDescending(kv => kv.Value).First().Key;
        }
...
foreach (var (type, content) in blocks)
{
    switch (type)
    {
        ...
        case "section":
            bodyTex.AppendLine($"\\section{{{EscapeLatex(content)}}}");
            bodyTex.AppendLine();
            break;

        case "subsection":
            bodyTex.AppendLine($"\\subsection{{{EscapeLatex(content)}}}");
            bodyTex.AppendLine();
            break;

        case "subsubsection":
            bodyTex.AppendLine($"\\subsubsection{{{EscapeLatex(content)}}}");
            bodyTex.AppendLine();
            break;

        case "para":
            bodyTex.AppendLine(EscapeLatex(content));
            bodyTex.AppendLine();
            break;
        ...
        case "fig_caption":
            // 去除手动编号前缀（图3.1a、图3.1等），让 LaTeX 自动编号
            ...
            if (!string.IsNullOrEmpty(figCaption) && string.IsNullOrEmpty(_pendingFigCaption))
                _pendingFigCaption = EscapeLatex(figCaption);
            break;

        case "image":
            figureCounter++;
            bodyTex.AppendLine("\\begin{figure}[htbp]");
            bodyTex.AppendLine("  \\centering");
            bodyTex.AppendLine($"  \\includegraphics[width=0.8\\textwidth]{{figures/{content}}}");
            if (!string.IsNullOrEmpty(_pendingFigCaption))
            {
                bodyTex.AppendLine($"  \\caption{{{_pendingFigCaption}}}");
                _pendingFigCaption = "";
            }
            else
            {
                bodyTex.AppendLine("  \\caption{}");
            }
            bodyTex.AppendLine($"  \\label{{fig:f{figureCounter}}}");
            bodyTex.AppendLine("\\end{figure}");
            bodyTex.AppendLine();
            break;

        case "table_caption":
            _pendingTableCaption = EscapeLatex(content);
            break;
        ...
        case "table_data2":
        {
            ...
            // 使用 JSON 中预计算的 colspec 和 rowhead
            ...
            tableCounter++;
            bodyTex.AppendLine($"\\begin{{longtblr}}[");
            if (!string.IsNullOrEmpty(_pendingTableCaption))
            {
                bodyTex.AppendLine($"  caption = {{{_pendingTableCaption}}},");
                _pendingTableCaption = "";
            }
            bodyTex.AppendLine($"  label = {{tab:t{tableCounter}}},");
            bodyTex.AppendLine("]{");
            bodyTex.AppendLine($"  colspec = {{{colSpec}}},");

            // 表格级字号
            double tFz = jsonDoc.RootElement.TryGetProperty("tableFz", out var fzProp) ? fzProp.GetDouble() : 0;
            if (tFz > 0 && Math.Abs(tFz - 12) > 0.5)
                bodyTex.AppendLine($"  rows = {{font=\\fontsize{{{tFz}pt}}{{1.2\\baselineskip}}\\selectfont}},");
            else if (colCount >= 8)
                bodyTex.AppendLine("  rows = {font=\\small},");

            bodyTex.AppendLine($"  rowhead = {{{headerRows}}},");
            bodyTex.AppendLine("  hlines, vlines,");
            bodyTex.AppendLine("}");

            for (int ri = 0; ri < allRows.Count; ri++)
            {
                var parts = new List<string>();
                for (int ci = 0; ci < allRows[ri].Count; ci++)
                {
                    var (text, cs, rs, bg) = allRows[ri][ci];
                    if (string.IsNullOrEmpty(text) && cs == 1 && rs == 1 && string.IsNullOrEmpty(bg))
                    { parts.Add(""); continue; }

                    if (cs > 1 || rs > 1 || !string.IsNullOrEmpty(bg))
                    {
                        var opts = new List<string>();
                        if (rs > 1) opts.Add($"r={rs}");
                        if (cs > 1) opts.Add($"c={cs}");
                        // TODO: 底色暂未启用（tabularray bg 语法兼容性）
                        // if (!string.IsNullOrEmpty(bg)) opts.Add($"bg=#FF{bg}");
                        parts.Add($"\\SetCell[{string.Join(",", opts)}]{{c}} {FormatCellText(text)}");
                    }
                    else
                    {
                        parts.Add(FormatCellText(text));
                    }
                }
                bodyTex.AppendLine($"  {string.Join(" & ", parts)} \\\\");
            }

            bodyTex.AppendLine("\\end{longtblr}");
            bodyTex.AppendLine();
            break;
        }
    }
...
// 第4步：写出文件
// ══════════════════════════════════════════════════
string GenerateFormatSty()
{
    return @"% format.sty
\ProvidesPackage{format}
\RequirePackage[fontset=windows]{ctex}
\RequirePackage[a4paper, left=2.5cm, right=2.5cm, top=3cm, bottom=2.5cm, headheight=15pt, footskip=1.5cm]{geometry}
\RequirePackage{graphicx}
\RequirePackage{tabularray}
% 全网格线
\SetTblrInner{hlines, vlines}
% 所有单元格文字居中（水平+垂直）
\SetTblrInner{cells = {c, m}}
...
% 表格伸缩（需要时调整数值，建议 1.03~1.08）
% \SetTblrInner{stretch=1.0}
...
string FormatCellText(string text)
{
    // text is already EscapeLatex'd; insert break points for narrow columns
    text = Regex.Replace(text, @"(\\textasciitilde\s)(\d)", "$1\\allowbreak{}$2");
    text = Regex.Replace(text, @"(\d)-(\d)", "$1-\\allowbreak{}$2");
    text = Regex.Replace(text, @"(\d)/(\d)", "$1/\\allowbreak{}$2");
    return text;
}

// ── 写文件 ──
File.WriteAllText(Path.Combine(outputDir, "format.sty"), GenerateFormatSty());
Console.WriteLine("  format.sty");

string mainTex = $@"% {EscapeLatex(fileName)} — 自动转换自 DOCX
...
% === 正文开始 ===
{bodyTex.ToString().TrimEnd()}

\end{{document}}
";
File.WriteAllText(Path.Combine(outputDir, "main.tex"), mainTex);
Console.WriteLine("  main.tex");

File.WriteAllText(Path.Combine(outputDir, "ref.bib"), "% 参考文献\n");
Console.WriteLine("  ref.bib");

File.WriteAllText(Path.Combine(outputDir, ".latexmkrc"), @"$pdf_mode = 5;   # 5 = xelatex
$xelatex = 'xelatex %O %S';
");
Console.WriteLine("  .latexmkrc");
...
File.WriteAllText(Path.Combine(vsDir, "settings.json"), @"{
  ...
  ""latex-workshop.latex.tools"": [
    ...
    },
    {
      ...
      ""command"": ""latexmk"",
      ""args"": [""-xelatex"", ""-f"", ""-synctex=1"", ""-interaction=nonstopmode"", ""-file-line-error"", ""%DOC%""]
    }
  ],
...
Console.WriteLine("\n✅ 转换完成！");
Console.WriteLine($"   编译: cd \"{outputDir}\" && latexmk -xelatex main.tex");
return 0;
